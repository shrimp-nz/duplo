...
#define MUSTADDED 		void operator delete(void* p, size_t s) { \
	...
	public:
		...
		bool destroy(void(*free_callback)(const T* item)=nullptr);
	...
	inline bool kavl<T>::init(PRTL_AVL_COMPARE_ROUTINE cmp_func)
	{
		...
		__lock = reinterpret_cast<PERESOURCE>(ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof ERESOURCE, pool_tag));
		if (!__lock) return false;
		__avl_table = reinterpret_cast<PRTL_AVL_TABLE>(ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof RTL_AVL_TABLE, pool_tag));
		if (!__avl_table) {
			ExFreePool(__lock);
			return false;
		}
	...
	template<typename T>
	inline bool kavl<T>::destroy(void(*free_callback)(const T* item))
	{
