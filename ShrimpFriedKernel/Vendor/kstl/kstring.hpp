...
namespace kstd {
	...
	public:
		...
		basic_string(const PUNICODE_STRING rhs);
		~basic_string();
	private:
		T* __data;
	...
	inline T* basic_string<T>::__alloc(size_t bytes)
	{
		return reinterpret_cast<T*>(
			ExAllocatePool2(POOL_FLAG_NON_PAGED, bytes, 'kstr')
			);
	}
	...
	template<>
	inline basic_string<wchar_t>::basic_string(const PUNICODE_STRING str) :__data(0), __size(0) {
		if (!MmIsAddressValid((PVOID)str)) return;
