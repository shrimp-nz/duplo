...
#define KDDEBUGGER_DATA_OFFSET 0x2080
	...
	inline SysInfoManager* kstd::SysInfoManager::getInstance(){
		...
		do {
			...
			__instance = reinterpret_cast<SysInfoManager*>(ExAllocatePool2(POOL_FLAG_NON_PAGED,
...
#define DUMP_BLOCK_SIZE 0X40000
			...
			tmp = reinterpret_cast<char*>(ExAllocatePool2(POOL_FLAG_NON_PAGED, DUMP_BLOCK_SIZE, 'sysI'));
			if (tmp == nullptr) {
				break;
			}
			CONTEXT context = { 0 };
	...
	inline void SysInfoManager::destroy()
	{
		if (__instance != nullptr) {
			ExFreePool(__instance);
		}
