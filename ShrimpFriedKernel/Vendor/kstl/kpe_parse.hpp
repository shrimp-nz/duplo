...
#define MAXIMUM_FILENAME_LENGTH 256
	...
	inline ParsePE::ParsePE(const wchar_t* file_path):__h_file(0),__base(0),__size(0),__noclean_base(0),_base(0)
	{
		...
		do {
			...
			pe_buf = ExAllocatePool2(POOL_FLAG_NON_PAGED, __size, pool_tag);
			if (pe_buf == nullptr) {
				break;
			}
			...
			auto opt_header = nt_headers->OptionalHeader;
			//开始map
			__base = ExAllocatePool2(POOL_FLAG_NON_PAGED, opt_header.SizeOfImage, pool_tag);
			if (!__base) {
				break;
			}
	...
	inline ParsePE& ParsePE::operator=(const ParsePE& rhs)
	{
		...
		if (MmIsAddressValid(rhs.__base)) {
			...
			__base = ExAllocatePool2(POOL_FLAG_NON_PAGED, rhs.__size, pool_tag);
			if (__base) {
