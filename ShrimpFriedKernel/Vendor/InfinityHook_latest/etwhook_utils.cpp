...
kstd::kwstring getModuleNameByPtr(PVOID p, PVOID* base, size_t* size) {
	kstd::kwstring find_name{ L"unknown module" };
	ULONG needSize = 0;
	ZwQuerySystemInformation(SystemModuleInformation, nullptr, 0, &needSize);
	needSize += PAGE_SIZE;
	wchar_t* wstr = nullptr;

	const auto info = reinterpret_cast<SYSTEM_MODULE_INFORMATION*>(ExAllocatePool2(POOL_FLAG_NON_PAGED, needSize, 'temp'));
	if (info == nullptr) {
		LOG_ERROR("failed to allocate memory for sys infomation!\r\n");
		return find_name;
	}

	wstr = reinterpret_cast<wchar_t*>(ExAllocatePool2(POOL_FLAG_NON_PAGED, 512 * 2, 'temp'));
	if (wstr == nullptr) {
		LOG_ERROR("failed to allocate memory for str\r\n");
		return find_name;
	}
...
auto find_module_base(const wchar_t* w_module_name, ULONG* size) -> void* {
	...
	auto info = reinterpret_cast<SYSTEM_MODULE_INFORMATION*>(
		ExAllocatePool2(POOL_FLAG_NON_PAGED, needSize, 'temp'));
